...
([Aquí](https://tidyr.tidyverse.org/reference/pivot_longer.html#examples) algunos ejemplos más)
...
```{r,eval=FALSE,echo=TRUE}
p <- ggplot(mpg, aes(displ, cty)) + geom_point() + theme_bw()
```
...
- Necesitas agregar la __función `annotate()`__ con el `geom` que quieres usar y las coordinadas (x,y). 
