...
date: "`r Sys.Date()`<br/><br/>(adaptado al español por Andree Valle Campos)"
output:
...
2. Cada __observation__ es una __fila__.

.pull-left[

#### Formato ancho ("sucio") 

Droga | Respuesta.CTRL | Respuesta.T1D
-----|---------------|----------------
A	| 3	| 12
B | 6 |	56
C	| 8 |	2
...
.right-column[
Ajusta las escalas para los diferentes parámetros estéticos (Mapeos):

`scale` + `<aes>` + `<type>`, donde:

- `<aes>` indica el parámetro que quieres modificar (_x_, _y_, _color_, _shape_)
- `<type>` indica qué tipo de variable `<aes>` es (o la modificación a realizar).

Algunos ejemplos:

- `scale_x_discrete()` modifica el __eje x__ discreto.
- `scale_y_continuous()` modifica el __eje y__ continuo. 
- `scale_color_grey()` modifica el __color__ a una escala de grises.
- `scale_fill_gradient()` ajusta el color de __relleno__ en gradiente.
]
...
p + labs(title = "RColorBrewer", subtitle = "p") +
  (p + scale_color_viridis_c() + labs(title = "viridis",
                                      subtitle = "p+scale_color_viridis_c()")) +
  (p + colorspace::scale_color_continuous_diverging(palette = "Blue-Red",
                                                 mid = 0.5) +
     labs(title = "colorspace", 
          subtitle = "p+scale_color_continuous_diverging(mid=0.5)")) +
...
.right-column[
La función `stat` y parámetros realizar transformación en tu data, por ejemplo:
...
.right-column[
El **sistema de coordenadas** define cómo el ejes x y eje y se combinan a posicionar los elementos del gráfico.

El sistema de coordenadas por defecto es el **cartesiano** (`coord_cartesian()`) y varias funciones pueden modificarlo:
- `coord_fixed()` mantiene el mismo "aspect ratio" para eje x y eje y.
- `coord_flip()` gira 90° el eje de coordenadas.

También pueden transformar tu data a coordenadas polares con `coord_polar()`.
]
...
- `facet_grid()`. Muestra paneles en una grilla usando __dos__ variables discretas (`variable1 ~ variable2`).
- `facet_wrap()`. Crea una serie de paneles mostrando seleciones de diferentes valores para tu variable discreta.
...
.right-column[
Permite la anotación de diferentes tipos de `geom` manualmente dando su posición en eje x y eje y.

Solo necesitas agregar la función `annotate()` con el `geom` que quieres usar y las coordinadas eje x y eje y, donde serán dibujadas. 
...
La función  `ggsave()` permite grabar los gráficos ggplot

Por defecto, grabará el último gráfico generado en el archivo especificado en `filename`.

Automáticamente extraerá el formato correcto del sufijo en `filename` ("png", "pdf", "svg" entre otros)
