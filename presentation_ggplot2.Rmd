...
.right-column[
El argumento __mapping__ indica cómo mapeamos la columna de datos a elementos visuales o parámetros __dentro de__ la función `aes()`:
...
# para geometría específica
ggplot(iris, aes(x = Sepal.Length, y = Sepal.Width)) +
  geom_point() +
{{geom_line(aes(color=Species))}}

# ¡Veremos el resultado luego de 3 diapositivas!
```
]
...
.right-column[
Así quedarían las **geometrías** al usar el **mapeo** completo y específico:
...
```{r mapeo01,echo=FALSE,include=FALSE, fig.show="hide", fig.dim=c(8.6, 2.3), dev="svg"}
# para gráfico completo o global
...
p + labs(title = "RColorBrewer", subtitle = "p") +
  ...
  (p + colorspace::scale_fill_continuous_diverging(palette = "Blue-Red",
     ...
     labs(title = "colorspace", 
          subtitle = "p+scale_fill_continuous_diverging(mid=0.5)")) +
...
.right-column[
La función `stat` y sus parámetros realizan la transformación en tu data, por ejemplo:
...
library(patchwork)
p + labs(title = "p") +
  (p + facet_grid(cyl~drv) + labs(title = "p + facet_grid(cyl~drv)")) +
  (p + facet_wrap(~cyl) +
     labs(title = "p + facet_wrap(~cyl)" )) +
